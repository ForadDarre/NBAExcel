...
namespace NBAExcel
{
	public class HandicapCollection
	{
		...
		public static void Main()
		{
			...
			{
				int j = 0;
				for(double i = -30.0; i <= 30.0; i += 0.5, j++)
				{
		...
		{
			string value = (oddsSeason.Cells[row, 13] as Excel.Range).Text;
			if (double.TryParse(value, out double handicapVar))
			{
				if (handicapVar == handicap)
				{
					double handicapOddsA = double.Parse((oddsSeason.Cells[row, 14] as Excel.Range).Text);
		...
		private static void CopyCoefsToConvStats(Excel.Workbook workbook, List<List<double>> allCoefs, double handicap, int column)
		{
			...
			{
				convStats.Cells[i + 4, column] = allCoefs[i][0];
